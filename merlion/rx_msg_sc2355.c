...
static int sprdwl_pkt_log_dump(void *data)
{
	struct sprdwl_pktlog_hdr *hdr = NULL;
	struct sprdwl_pktlog_info *pkt_info = NULL;
	int plen = 0, len = 0;
	unsigned char *buf = NULL;

	buf = (unsigned char *)data;
	hdr = (struct sprdwl_pktlog_hdr *)buf;
	plen = SPRDWL_GET_LE16(hdr->plen);
	buf += sizeof(struct sprdwl_pktlog_hdr);

	while (plen > 0) {
		pkt_info = (struct sprdwl_pktlog_info *)buf;
		len = SPRDWL_GET_LE16(pkt_info->len) - sizeof(struct sprdwl_pktlog_info);
		plen -= sizeof(struct sprdwl_pktlog_info);
		buf += sizeof(struct sprdwl_pktlog_info);

		wl_err("[%u] CP2_PKT type[0x%02x], subtype[0x%02x], len = %d, from = %d\n", SPRDWL_GET_LE32(pkt_info->mstime),
			pkt_info->type, pkt_info->sub_type, len, SPRDWL_GET_LE16(pkt_info->pkt_from));
		print_hex_dump(KERN_ERR, "CP2_PKT: ", DUMP_PREFIX_OFFSET, 16, 1, buf, len, 0);
	...
	while (1) {
		...
		while ((msg = sprdwl_peek_msg_buf(&rx_if->rx_list))) {
			...
			case SPRDWL_TYPE_PKT_LOG:
				if (msg->len > SPRDWL_MAX_CMD_RXLEN)
					wl_err("err pkt log data trans too long:%d > %d\n",
							msg->len, SPRDWL_MAX_CMD_RXLEN);
				sprdwl_pkt_log_dump(msg->data);
				break;
			default:
