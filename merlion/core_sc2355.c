...
#ifdef CHR_SUPPORT
			CHR_OPENERR_FLAGSET(&intf->priv->chr->open_err_flag, OPEN_ERR_POWER_ON);
#endif
			return ret;
		}
		if (atomic_read(&intf->power_cnt) == 1)
...
int sprdwl_probe(struct platform_device *pdev)
{
...
#ifdef CHR_SUPPORT
	chr = sprdwl_chr_handle_probe(priv);
	if (!chr) {
	...
	if (ret) {
#ifdef CHR_SUPPORT
		sprdwl_chr_deinit(chr, PROBE_DEINIT);
#endif
		sprdwl_tx_deinit(intf);
		sprdwl_rx_deinit(intf);
#ifdef SIPC_SUPPORT
...
int sprdwl_remove(struct platform_device *pdev)
{
	...
	if (ret)
		return ret;
#ifdef CHR_SUPPORT
	sprdwl_chr_deinit(priv->chr, REMOVE_DEINIT);
#endif
