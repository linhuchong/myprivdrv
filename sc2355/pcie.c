		       ...
		       int tx_count, int ac_index, u8 coex_bt_on)
{
	...
	if (!__list_del_entry_valid(tx_list)){
		pr_info("%s to_send_list is deleted !\n",__func__);
		return -1;
	}
...
void pcie_mm_fill_all_buffer(struct sprd_hif *hif)
{
	...
	atomic_set(&mm_entry->flush_flag, 0);
}
...
int pcie_init(struct sprd_hif *hif)
{
	...
	sprd_throughput_static_init(hif->priv);
	tx_mgmt = (struct tx_mgmt *)hif->tx_mgmt;
	//reset thread uclamp param
	sc2355_set_thread_uclamp(tx_mgmt->tx_thread, 0);
...
{
	sprd_throughput_static_deinit(hif->priv);
...
static struct sprd_hif_ops sc2355_pcie_ops = {
	...
	.throughput_ctl_pd = sc2355_throughput_ctl_core_pd,
