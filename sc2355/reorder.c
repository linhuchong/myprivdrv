			...
			bool *full_msdu_flag)
{
	...
	if (seqno_geq(seq_num, ba_node_desc->win_start)) {
		...
		pr_err("%s: seq_num: %d is less than win_start: %d, tid=%d\n",
...
static void reorder_del_ba_node(struct rx_ba_entry *ba_entry,
				struct rx_ba_node *ba_node)
{
	pr_info("%s: del ba lut_index: %d, tid %d, active %u\n",
		__func__, ba_node->sta_lut_index, ba_node->tid, ba_node->active);
			...
			unsigned int size)
{
	...
	if (ba_node) {
		/*
		 * 2384596
		 * if new index_size  != index_size in current ba_node,
		 * free ba_node and realloc ba_node with new index_size.
		 */
		if (ba_node->rx_ba->index_mask + 1 != size) {
			pr_info("%s,realloc ba_node:old index_size(%d) new index_size(%d)\n",
				__func__, (ba_node->rx_ba->index_mask + 1), size);
			reorder_del_ba_node(ba_entry, ba_node);
		} else {
			return ba_node;
		}
	}
				   ...
				   struct evt_ba *ba_event)
{
	...
	if (ba_node->active) {
		...
		pr_info("%s lut_idx-tid %u-%u (win size:%d start:%d tail:%d)\n",
...
static void reorder_ba_timeout(struct timer_list *t)
{
	...
	pr_info("enter %s lut_idx-tid %u-%u active %u\n", __func__,
				  ...
				  unsigned short len)
{
	...
	pr_info("%s ba_event type: %d, lut_index-tid: %u-%u.\n", __func__, type,
		ba_event->sta_lut_index, ba_event->tid);
	switch (type) {
